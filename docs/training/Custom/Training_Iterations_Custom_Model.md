...
- **Dataset Quality:**
  - moved from manually Patch-by-Patch labelling to Roboflow labeling. 
  - this increased the number of patches significantly. Now training with 100.000 patches instead of 40.000
  - rebalanced the dataset. Since in a sliding-window approach the model sees far more negative patches than negatives, the false positive rate is of utmost importance. Because of that the dataset now contains 3 times as many negative patches as positives.
  - The synthetic images were also improved significantly.

- **Model:**
  - Same as last iteration. Still training on grayscale images.
